...
`--adaptive` / `--gpu-ratio adaptive` 当前只是占位策略，默认按 `0.5` ratio 执行；真正的自适应调度还需要后续基于 daemon/bench 的比例扫描。
...
- `0 < --gpu-ratio < 1`：按连续 block range 切分到 GPU/CPU。
- `--cpu-threads N`：限制 OpenCL CPU device 的 worker/slot 数。
- 容器格式与 `lz4_gpu_v2` 一致，压缩输出可由同实现解压回放。
...
2. 在比例扫描成立后再实现真正 adaptive 调度。
3. 更新 `bench_lz4.py`，把 `lz4_hybrid` 纳入正式全样本测试矩阵。
