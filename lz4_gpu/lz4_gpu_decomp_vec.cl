...
inline U32 LZ4_NbCommonBytes64(U64 val) {
    ...
    U32 n = 0;
    if ((val & 0xFFFFFFFFULL) == 0ULL) { n += 4; val >>= 32; }
    if ((val & 0xFFFFULL) == 0ULL) { n += 2; val >>= 16; }
    if ((val & 0xFFULL) == 0ULL) { n += 1; }
    return n;
}
        ...
        if (cpy > oend - MATCH_SAFEGUARD_DISTANCE) {
            ...
            if (offset < 8) {
                if (offset == 1) {
                    ...
                    /* fallback: preserve baseline semantics for offset 3 */
                    op[0] = match[0]; op[1] = match[1]; op[2] = match[2]; op[3] = match[3];
                    match += inc32table[offset];
                    LZ4_memcpy(op + 4, match, 4);
                    match -= dec64table[offset];
                } else if (offset == 5) {
                    /* fallback: preserve baseline semantics for offset 5 */
                    U32 v0 = LZ4_read32(match);
                    LZ4_write32(op, v0);
                    match += inc32table[offset];
                    LZ4_memcpy(op + 4, match, 4);
                    match -= dec64table[offset];
                } else if (offset == 6) {
                    /* Preserve baseline semantics for offset 6 */
                    U32 v0 = LZ4_read32(match);
                    LZ4_write32(op, v0);
                    match += inc32table[offset];
                    LZ4_memcpy(op + 4, match, 4);
                    ...
                    /* fallback: preserve baseline semantics for offset 7 */
                    op[0] = match[0]; op[1] = match[1]; op[2] = match[2]; op[3] = match[3];
                    match += inc32table[offset];
                    LZ4_memcpy(op + 4, match, 4);
                    match -= dec64table[offset];
